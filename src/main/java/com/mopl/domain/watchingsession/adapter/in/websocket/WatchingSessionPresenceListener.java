 ...
 * (WatchingSessionUseCase.leaveIfCurrent). 그냥 watcherId만으로 지우면 탭Y가 엉뚱하게 퇴장 처리된다.
 */
@Slf4j
@Component
...
public class WatchingSessionPresenceListener {
  ...
  public void handleSubscribe(SessionSubscribeEvent event) {
    ...
    try {
      WatchingSessionDto dto = watchingSessionUseCase.enter(watcherId, contentId);
  ...
  private void leaveIfCurrent(StompHeaderAccessor accessor, String sessionId, UUID watchingSessionId) {
    ...
    try {
      ...
      watchingSessionUseCase.leaveIfCurrent(watcherId, watchingSessionId);
